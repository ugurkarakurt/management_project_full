...
function withDashboardAlignment<P extends object>(
  ...
  const WithDashboardAlignment = (props: Omit<P, '$alignment'>) => {
    const { alignment } = useDashboardCardAction();
    return <WrappedComponent {...props as P} $alignment={alignment} />;
  };

  WithDashboardAlignment.displayName = `WithDashboardAlignment(${WrappedComponent.displayName || WrappedComponent.name || 'Component'
    })`;

  return WithDashboardAlignment;
}
